  ...
  if(Serial.available())
    {
       esp_data = Serial.readStringUntil('\n');
        ...
        int cordinatecount = 0;            
        
        for (int i=0; i < esp_data.length(); i++)
        { 
            if(esp_data.charAt(i)== '|')
            {        
              cordinatecount += 1;      
              continue;
            }
            
            Cordinates[cordinatecount] = Cordinates[cordinatecount] + esp_data.charAt(i);            
        }
        ...
        {  
          ...
          for (int i=0; i < cordinatecount; i++)
           {
              if(motor_ran == true)
              {
                motor_ran = false;
                Serial.print(Cordinates[i]);
                Serial.println();
                getinput(Cordinates[i]);
              }
           }
